...
pub async fn update_developer_profile(
    ...
    sqlx::query(
        "UPDATE developer_profiles SET bio = ?, tech_stack = ?, github_url = ?, linkedin_url = ?, level = ? WHERE user_id = ?"
    )
    .bind(bio)
    .bind(&tech_stack)
    .bind(github_url)
...
pub async fn update_company_profile(
    ...
    sqlx::query(
        "UPDATE company_profiles SET company_name = ?, description = ?, website = ?, size = ?, tech_stack = ? WHERE user_id = ?"
    )
    .bind(company_name)
    .bind(description)
    .bind(website)
