    ...
    <div>
      ...
      {data?.data.length === 0 && (
        <p className="text-gray-500">
          ...
          <Link to="/listings/new" className="text-primary-600 hover:text-primary-500">
            Create one
          </Link>
        </p>
      )}
        ...
        {data?.data.map((listing) => (
          <div key={listing.id} className="bg-white border border-gray-200 rounded-lg p-4">
            <div className="flex flex-col sm:flex-row items-start sm:items-center sm:justify-between gap-2">
              <div>
                <Link to={`/listings/${listing.id}`} className="font-medium text-gray-900 hover:text-primary-600">
                  {listing.title}
                </Link>
              ...
              {listing.status === 'active' && (
                <div className="flex gap-2">
                  <button
                    ...
                    className="text-sm text-primary-600 hover:text-primary-800 border border-primary-200 px-3 py-1 rounded"
                  >
                    Edit
                  </button>
                  <button
